...
const AITravel = () => {
  ...
  const handleBookNow = (travel: Travel) => {
    requireAuth(() => {
      const interaction: UserInteraction = {
        ...
        favourite: false,
        booked: true,
        total: travel.price || 0,
      };

      interaction.total = calculateTotal(interaction);
      saveInteraction(interaction);

      navigate(`/book-now?travelId=${travel.id}`, {
        state: {
          travelId: travel.id,
        },
        replace: false,
            ...
            <div className="p-6 lg:p-8">
              ...
              <div className="flex space-x-4">
                <button 
                  ...
                  onClick={() => handleViewDetails(currentTravel)}
                >
                  View Details
                </button>
                <button 
                  className="flex-1 bg-white border border-orange-500 text-orange-500 py-3 px-6 rounded-lg hover:bg-orange-50 transition-all duration-200 text-lg font-semibold transform hover:scale-[1.02]"
                  onClick={() => handleBookNow(currentTravel)}
                  disabled={currentTravel.availableSeats <= 0}
                >
                  {currentTravel.availableSeats > 0 ? "Book Now" : "Sold Out"}
                </button>
              </div>
            </div>
          </div>
